...
$leaderboardProc = Start-Process go -ArgumentList "run ./cmd/leaderboard-service --port 8282" -PassThru -NoNewWindow
$bgJobs += $leaderboardProc

Write-Host "`n=== 3. Starting Matching Engine (Background) ===" -ForegroundColor Cyan
$engineProc = Start-Process go -ArgumentList "run ./cmd/matching-engine --port 8080" -PassThru -NoNewWindow
$bgJobs += $engineProc
